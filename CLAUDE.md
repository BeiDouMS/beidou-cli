...
- **CallCommand**: 支持 GET/POST/PUT/DELETE/PATCH。body 参数自动包装为 `SubmitBody { requestId: null, data: <body> }`。无 body 时 GET/DELETE 不发 body，其他方法发空 POST body。
...
- `beidou apis [keyword]` — 查看所有可用 API 及参数说明（共 55 个接口，定义在 `ApiListCommand.java`），支持关键词过滤
...
- 敏感判定逻辑在 `ApiListCommand.isSensitive()`：先按 API 列表中精确路径匹配（优先），未匹配时用正则回退匹配
- `/auth` 路径免登录，其余自动认证
